...
export const StockOpnameView: React.FC = () => {
  ...
  const filledLines = lines.filter((line) => Number.isFinite(line.physicalStock)).length;
  const differences = lines.filter((line) => Number.isFinite(line.physicalStock) && line.difference !== 0).length;
  ...
  const generateTemplate = () => {
    ...
    setLines(selected.map((item) => ({ itemId: item.id, sku: item.sku, itemName: item.name, unit: item.unit, rack: item.warehouseLocation, category: item.category, systemStock: item.currentStock, physicalStock: Number.NaN, difference: 0 })));
  };

  const updatePhysical = (index: number, rawValue: string) => {
    const physicalStock = rawValue.trim() === '' ? Number.NaN : Number(rawValue);
    setLines((current) => current.map((line, lineIndex) => lineIndex === index
      ? { ...line, physicalStock, difference: Number.isFinite(physicalStock) ? physicalStock - line.systemStock : 0 }
      : line));
  };
  ...
  return <div className="space-y-4 sm:space-y-5">
    ...
    {lines.length > 0 && <section className="overflow-hidden rounded-2xl border border-[#e2e0da] bg-white">
      <div className="flex items-center justify-between border-b border-[#eceae5] p-4"><div><h2 className="text-sm font-bold">Input hasil hitung</h2><p className="text-[10px] text-[#85847e]">{filledLines}/{lines.length} terisi · {differences} selisih</p></div><ClipboardCheck className="h-5 w-5 text-[#397c31]" /></div>
      <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[760px] text-left text-xs"><thead className="bg-[#faf9f6] text-[10px] uppercase text-[#85847e]"><tr><th className="p-3">Barang</th><th className="p-3">Rak</th><th className="p-3 text-right">Sistem</th><th className="p-3 text-right">Fisik</th><th className="p-3 text-right">Selisih</th></tr></thead><tbody>{lines.map((line,index)=><tr key={line.itemId} className="border-t border-[#efede8]"><td className="p-3"><strong>{line.itemName}</strong><p className="font-mono text-[10px] text-[#85847e]">{line.sku} · {line.category}</p></td><td className="p-3 text-[#666]">{line.rack}</td><td className="p-3 text-right">{line.systemStock} {line.unit}</td><td className="p-3"><NumberInput key={`desktop-${templateVersion}-${line.itemId}`} name={`physical-desktop-${templateVersion}-${line.itemId}`} autoComplete="off" type="number" min="0" placeholder="Masukkan" value={Number.isFinite(line.physicalStock) ? line.physicalStock : ''} onChange={(event)=>updatePhysical(index,event.target.value)} className="ml-auto h-10 w-28 rounded-lg border px-2 text-right font-bold" /></td><td className={`p-3 text-right font-bold ${!Number.isFinite(line.physicalStock) ? 'text-[#aaa8a1]' : line.difference ? 'text-[#bd4943]' : 'text-[#397c31]'}`}>{Number.isFinite(line.physicalStock) ? `${line.difference > 0 ? '+' : ''}${line.difference}` : '—'}</td></tr>)}</tbody></table></div>
      <div className="divide-y divide-[#efede8] md:hidden">{lines.map((line,index)=><article key={line.itemId} className="p-4"><div className="flex justify-between gap-3"><div className="min-w-0"><h3 className="truncate text-sm font-semibold">{line.itemName}</h3><p className="mt-1 text-[10px] text-[#85847e]">{line.sku} · {line.rack}</p></div><span className={`text-xs font-bold ${!Number.isFinite(line.physicalStock) ? 'text-[#aaa8a1]' : line.difference ? 'text-[#bd4943]' : 'text-[#397c31]'}`}>{Number.isFinite(line.physicalStock) ? `${line.difference > 0 ? '+' : ''}${line.difference}` : '—'}</span></div><div className="mt-3 grid grid-cols-2 gap-2"><div className="rounded-xl bg-[#f5f4f0] p-3"><span className="text-[10px] text-[#85847e]">Stok sistem</span><strong className="block text-sm">{line.systemStock} {line.unit}</strong></div><div><label className="text-[10px] text-[#85847e]">Hasil fisik</label><NumberInput key={`mobile-${templateVersion}-${line.itemId}`} name={`physical-mobile-${templateVersion}-${line.itemId}`} autoComplete="off" type="number" min="0" placeholder="Masukkan" value={Number.isFinite(line.physicalStock) ? line.physicalStock : ''} onChange={(event)=>updatePhysical(index,event.target.value)} className="mt-1 h-11 w-full rounded-xl border px-3 text-right font-bold" /></div></div></article>)}</div>
      <div className="sticky bottom-0 flex justify-end border-t bg-white p-3"><button onClick={submit} disabled={filledLines !== lines.length} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#252525] px-5 text-xs font-bold text-white disabled:bg-[#d9d8d3] disabled:text-[#85847e] sm:w-auto"><Save className="h-4 w-4 text-[#82dd70]" />Selesaikan Opname</button></div>
    </section>}
