...
export const StoreTransferView: React.FC = () => {
  const { items, warehouses, storeTransfers, createStoreTransfer, confirmStoreTransfer } = usePurchasing();
  const { profile } = useAuth();
  const sourceOptions = warehouses.filter((item) => item.type === 'warehouse' && item.isActive);
  const storeOptions = warehouses.filter((item) => item.type === 'store' && item.isActive);
  const [sourceId, setSourceId] = useState(sourceOptions[0]?.id || '');
  const [destinationId, setDestinationId] = useState(storeOptions[0]?.id || '');
  ...
  const submit = () => {
    ...
    const destination = storeOptions.find((item) => item.id === destinationId);
  ...
  return <div className="space-y-4 sm:space-y-5">
    <header><h1 className="text-xl font-bold text-[#292929] sm:text-2xl">Transfer Store</h1><p className="mt-1 text-xs text-[#77766f]">Barang dikirim dan barang diterima dicatat terpisah agar stok tidak terhitung ganda.</p></header>
    <section className="rounded-2xl border border-[#e2e0da] bg-white p-4 shadow-sm">
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2"><select value={sourceId} onChange={(event)=>setSourceId(event.target.value)} className="h-11 min-w-0 rounded-xl border border-[#d8d6cf] bg-white px-2 text-xs font-semibold">{sourceOptions.map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select><ArrowRight className="h-4 w-4 text-[#85847e]"/><select value={destinationId} onChange={(event)=>setDestinationId(event.target.value)} className="h-11 min-w-0 rounded-xl border border-[#d8d6cf] bg-white px-2 text-xs font-semibold">{storeOptions.map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></div>
